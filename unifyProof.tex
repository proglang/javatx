...
\begin{theorem}
  ...
  \sigma(b)$}{
   For $\overline{(\sigma,  \ol{Y} \triangleleft \ol{P})} =
   \unify{}(\consSet, \Delta)}$ any $(\sigma, \Delta') \in \overline{(\sigma,
   \ol{Y} \triangleleft \ol{P})}$ is unifier of $\consSet$ with $\Delta \cup \Delta'$.
\end{theorem}
...
\begin{theorem}\label{theo:unifyCompleteness}
  \textbf{(Completeness):} The \unify{} algorithm calculates \red{the set} of general unifier\red{s} for the input set of constraints ($\consSet_{in}$).
\end{theorem}
