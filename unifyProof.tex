...
\begin{theoremAndi}\label{theo:unifyCompleteness}
  \textbf{(Completeness):} The \textbf{Unify} algorithm calculates a general unifier for the input set of constraints ($Cons_{in}$).
...
\begin{theoremAndi}\label{theo:unifyTermination}
  \textbf{(Termination):} The \textbf{Unify} algorithm terminates on every finite input set.
\end{theoremAndi}
The \textbf{Unify} algorithm gets called with a set of input constraints.
...
We will show that \textbf{Unify} terminates on each of those sets by showing,
...
\textit{Proof:}
The \textbf{Unify} algorithm reduces the amount of type variables with every iteration.
...
\begin{enumerate}
  ...
  then the $Eq$ set has no possible solution and \textbf{Unify} would terminate as fail $Uni = \emptyset$: \\
  $
  \begin{array}[c]{ll}
  \begin{array}[c]{l}
...
\item[Step 3] The third step of the \textbf{Unify} algorithm removes at least one type variable
...
Due to the fact that there is never added a fresh type variable during the \textbf{Unify} algorithm,
