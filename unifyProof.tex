...
\replaced{We assume there exists a unifier}{Let} $\sigma = \set {a_1 \mapsto T_1, \ldots , a_n \mapsto T_n}$ \replaced{for the input constraints,
which is}{be} the result of the \unify{} algorithm.
\replaced{This means}{We show} for every constraint in the input set $(a \lessdot b) \in \consSet_{in}$ and $(c \doteq d) \in \consSet_{in}$
\erased{this unifier will substitute all variables in a way that all constraints are satisfied}:
$\sigma(a) \olsub \sigma(b)$\replaced{,}{ and} $\sigma(c) = \sigma(d)$\\

We now \replaced{look}{consider} at each step of the \unify{} algorithm
which transforms the input set of constraints $C$ to a set $C'$"
If \erased{we assume the unifier} $\sigma$ is \replaced{correct for the set}{an
  unifier of } $C'$,
then \replaced{we can show that it will also be correct for the constraints}
{$\sigma$ is an unifier of  $C$, too}.
...
\erased{The type variable $b$ appears only in constraints of the form $a \lessdot b$,
...
So the only two other possible combinations for constraints containing $b$
would be $b \lessdot c$ and $c \lessdot b$.
% \begin{itemize}
% \item 
If $\set{a \lessdot b, b \lessdot c} \in Eq$ then $\set{a \doteq b, a \lessdot c} \in Eq'$.
% %After using the unifier $a \to b$ on $Eq'$: $\set{a \doteq b, b \lessdot c} \in Eq'$.
% \item 
If $\set{a \lessdot b, c \lessdot b} \in Eq$ then $\set{a \doteq b, c \lessdot b} \in Eq'$.
% %After using the unifier $a \to b$ on $Eq'$: $\set{a \doteq b, b \lessdot c} \in Eq'$.
% \end{itemize}}
...
\item[reduce] The \replaced{\texttt{reduce1} and \texttt{reduce2}}{\textbf{reduce}} rule is obviously correct under the FJ typing rules.
\end{description}
...
This means there are only \replaced{two}{four\ } kinds of constraints left:
