As \TFGJ omits some type annotations, we cannot use
...
Most rules stay the same as in FGJ with the following exceptions:
\todo[inline]{PT20220126: where is $\Pi,\Delta, \Gamma$ defined? \\
  For  GT-FIELD, the function \textit{fields} should be defined (same
  as FGJ).\\
  GT-NEW is not correct because the instantiation is ignored.\\
  GT-INVK: $\Pi$ should be your substitute for \textit{mtype} and
  class-local methods should be recognized differently. In particular,
  in GT-CLASS, we need an incoming $\Pi$ and an outgoing $\Pi$. }
    ...
    \begin{minipage}{\textwidth}
      \begin{small}
        ...
        \begin{tabularx}{\textwidth}{X c X r}
          ...
          & & & \\
          & $\mathtt{ \Delta \vdash \texttt{Object}\ \text{ok}
          }$ & & WF-OBJECT\\
          & \\
          &
          ...
          & $\mathtt{\ddfrac{\begin{array}{c}
                               ...
                               \mathtt{\Delta} \vdash \ol{T} \ \text{ok}
...
$\mathtt{\ddfrac{\Gamma \vdash e_0:T_0 \quad \quad \mathit{fields}(\mathit{bound}_\Delta(T_0)) = \overline{T} \ \overline{f}}
...
$\mathtt{\ddfrac{\begin{array}{c}
  \mathtt{\exptype{}{\ol{Y} \triangleleft \ol{P}} \ol{U} \to U \in \mathit{mtype}(m, \mathit{bound}_\Delta (T_0))} \\
  \mathtt{\environmentvdash e_0 : T_0 } \quad \quad
  \mathtt{\Delta \vdash \ol{V} \ \texttt{OK} } \quad \quad
  \mathtt{\Delta \vdash \ol{V} \subeq  [\ol{V}/\ol{Y}]\ol{P} } \\ %\quad \quad
...
\begin{minipage}{\textwidth}
  \begin{small}
  \textbf{Method type lookup (Peter):} \\[1em]
...
\begin{tabularx}{\textwidth}{c}
  $\ddfrac{
    \textit{mtype}(m, N) = \exptype{}{\ol{Y} \triangleleft \ol{Q}} \ol{U} \to \mv U \ 
...
\begin{minipage}{\textwidth}
\begin{small}
\textbf{Method typing (Peter):}\\[1em]
\begin{tabularx}{\textwidth}{X c X r}
  & $\mathtt{\ddfrac{\begin{array}{c}
    \mathtt{\texttt{class}\ \exptype{C}{\ol{X} \triangleleft \ol{N}} \triangleleft N \{ \ldots\ \ol{M}\ \ldots\}}\\
                       \mathtt{
  \mathtt{\forall  \ol{T}, T: \ol{T} \to T \in \Pi(\exptype{C}{\ol{X}}.m) }\quad
  ...
  \texttt{ OK in }\exptype{C}{\ol{X} \triangleleft \ol{N}} \texttt{
    with } \exptype{}{\ol{Y} \triangleleft  \ol{P}} }
  }}$ & & \rulename{GT-METHOD}\\
\end{tabularx}\\[1em]
\textbf{Class typing (Peter):}\\[1em]
  \begin{tabularx}{\textwidth}{X c X r}
  %GT-CLASS: - This rule is modified by us
  & $\ddfrac{
    \begin{array}{c}
      \mathtt{\Pi' = \Pi \cup \set{\exptype{C}{\ol{X}}.m \mapsto \ol{T_m} \to T_m \,
      |\, m \in \ol{M}} } \\
      ...
      \exptype{C}{\ol{X} \triangleleft \ol{N}}  \texttt{
    with } \exptype{}{\ol{Y} \triangleleft  \ol{P}}}\\
  ...
  {\mathtt{ \Pi \vdash \texttt{class}\ \exptype{C}{\ol{X}
      ...
      \overline{f}; \ K \ \overline{M} \} \ \texttt{OK} \rhd \Pi''}}
  $ & & \rulename{GT-CLASS}
\end{tabularx}
\end{small}
\end{minipage}
...
%\begin{align*}
%\ddfrac{\begin{array}{c}
%\Delta \vdash \ol{X} \subeq  \ol{N}, \ol{Y} \subeq  \ol{P} \quad \quad 
...
%then $\environmentvdash e':T'$ for some $T'$ such that $\Delta \vdash T' \subeq  T$.
%\end{theoremAndi}
