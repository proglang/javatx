...
Figure \ref{fig:globalTI-example} shows a simplified example program.
It shows a simple implementation of a linked list.
It is a toy example, which has no real world application due to the fact that there is no neutral element for the linked list.
Also we leave out constructor for the sake of clarity.
...
\end{lstlisting}    
\caption{Global type inference example}
\label{fig:globalTI-example}
\end{figure}
