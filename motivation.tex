...
It is not impossible to  force GTI to infer the intended typing, but it requires some awkward coding.
\begin{lstlisting}
class Function<S,T> {
  S in;
  T out;
...
\item hence \texttt{T1 <: T} and \texttt{S <: S1} (because
  \texttt{this.val : S}),
\end{itemize}
  \todo[inline]{why is this resolved to \texttt{T1 = T} and
    \texttt{S = S1}?}
and resolves them to the desired outcome.
\begin{lstlisting}
class Box<S> {
  S val;
