...
\forall\ol{X}_i. \ol{A}_i \to \ol{A}_i$, for $1\le i\le n$. Define the \emph{instantiation
  graph $IG(\mv{C})$} as a directed graph with vertices $\{1, \dots, n\}$ and labeled edges
\begin{gather*}
  i \stackrel{\ol{U}/\ol{X}_j}{\longrightarrow} j
\end{gather*}
if $\mv{m}_i$ invokes $\mv{m}_j$ where the generic type variables in
the type of $\mv{m}_j$ are instantiated with substitution
$\ol{U}/\ol{X}_j$ (cf.\ rule GT-INVK).

Define the \emph{closure of the instantiation graph $IG^*(\mv{C})$} as the graph
obtained from $IG(\mv{C})$ by applying the following rule
exhaustively:
\begin{gather*}
  i \stackrel{\ol{U}/\ol{X}_j}{\longrightarrow} j
  \quad\wedge\quad
...
We say that method $\mv{m}_i$ is involved in polymorphic recursion if
there is an edge
\begin{gather*}
  i \stackrel{\ol{W}/\ol{X}_i}{\longrightarrow} i
  \quad \in IG^* (\mv{C})
  \qquad \text{such that}
  \qquad \ol{W} \ne \ol{X}_i
\end{gather*}
...
the graph $IG^* (\mv{UsePair})$ which indicates that \mv{prc} is
...
For \mv{UsePair2}, we obtain a graph $IG^* (\mv{UsePair2})$ with
