...
\begin{itemize}
\item From \texttt{p.fst}: $\mv P \subeq \mathtt{Pair}\Angle{\TVX,\TVY}$ and
  $\texttt{p.fst} : \TVX$.
\item From \texttt{p.snd}: $\mv P \subeq \mathtt{Pair}\Angle{\TVZ,\TVW}$ and
  $\texttt{p.snd} : \TVW$.
\item The two constraints on $\mv P$ imply that $\TVX = \TVZ$ and
  $\TVY = \TVW$.
\item From \texttt{.mult (p.snd)}: $\TVX \subeq \mathtt{Int}$, $\TVY \subeq
  \mathtt{Int}$, and $\mathtt{Int} \subeq \mv R$.
\end{itemize}
...
\begin{itemize}
\item From \texttt{.mult (p.snd)}: $\TVX \subeq \mathtt{MultPair}$, $\TVY \subeq
  \mathtt{P}$, and $\mathtt{R} \subeq \mv R$.
\end{itemize}
Transitivity of subtyping applied to $\TVY \subeq \mathtt{P}$ and $\mv P \subeq \mathtt{Pair}\Angle{\TVX,\TVY}$
yields the constraint $\TVY \subeq \mathtt{Pair}\Angle{\TVX,\TVY}$, which triggers the
...
we obtain the constraints \texttt{Bool $\subeq$ T} and \texttt{S $\subeq$ Bool} for \texttt{T = Bool}
and \texttt{S = Bool}, which are both satisfiable. With
\texttt{Negate} we run into the same situation with the constraints
\texttt{Int $\subeq$ Int} and \texttt{Int $\subeq$ Int}.
...
\item hence \texttt{S1 $\subeq$ T} and \texttt{S $\subeq$ S1}.
\end{itemize}
...
\texttt{Global $\subeq$ T} is not a valid constraint in FGJ.
