...
& \consSet_f = \set{\set{\type{R} \lessdot \exptype{C}{\ol{X}},\ a \doteq T} \ |\ \exptype{C}{\ol{X}}.\texttt{f} : \type{T} \in \ol{\fieldAssumption}}\\
& \constraints = \orCons(\constraints_{m})\\
\multicolumn{2}{l}{\mathbf{in}
(\tv{a}, [\textbf{fresh}(\ol{X})/\ol{X}](\consSet_e \cup \constraints))}\\
...
& \forall e_i \in \overline{e} : (pt_i, \constraints_i) = \typeExpr(\texttt{e}_i)  ,\\
& \begin{array}{@{}l@{}l}
  \constraints_{m} = \set{ & [\textbf{fresh}(\ol{Y})/\ol{Y}]\set{\type{R} \lessdot \exptype{C}{\ol{X}}, \tv{a} \doteq \type{T}, \bigcup_{T_i \in \overline{T}} (pt_i \lessdot T_i) , \ol{Y} \lessdot \ol{P}}\\
  & \ |\ (\exptype{C}{\ol{X}}.\texttt{m} : \exptype{}{\ol{Y} \triangleleft \ol{P}} \ \ol{T} \to T) \in \ol{\methodAssumption} }
\end{array}\\
& \constraints = \orCons(\constraints_{m})\\
\multicolumn{2}{l}{\mathbf{in} (a, [\textbf{fresh}(\ol{X})/\ol{X}](\constraints_R \cup \bigcup_i \constraints_i \cup \constraints))}\\
& \text{where\ } a \text{\ is\ a\ fresh\
  type\ variable}\\ 
\end{array}
\end{array}
