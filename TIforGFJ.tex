...
\todo[inline]{PT: define the syntax of constraints and constraint sets
  formally, starting with the syntax for a type variable. Introduce
  metavariables that are used consistently for single constraints (e.g., $c$) and
  constraint sets (e.g., $C$).  Are there
  restrictions on what is on the left/right side of a constraint?  An
  OrConstraint is a set of constraint sets, correct? }

\subsubsection{Syntax}
$\begin{array}{l}
%Type
\type{T} = \tv{a}\ |\ \gType{X}\ |\ \type{N} \\
%Constraints
c = \type{T} \lessdot \type{T} \ |\ \type{T} \doteq \type{T} \\
\constraint = c\ |\ \orCons \\
%\consSet = \set{C_1, \ldots, C_2} \\
\orCons{} = \set{\ol{c_1}, \ldots, \ol{c_n}} \\
% Method assumptions:
\methodAssumption = N.\texttt{methodName} : \exptype{}{\ol{X} \triangleleft \ol{N}}\ \ol{T} \to T \\
\fieldAssumption = N.\texttt{fieldName} : T \\
\localVarAssumption = \texttt{localVar} : T\\
