...
\quad \quad \begin{array}[t]{rll}
  \textbf{let}\ 
  (\overline{\methodAssumption}, \consSet) &= \fjtype{}(\mv{\Pi}, \texttt{L}) &
                                                                     \text{// constraint generation}\\
  \overline{(\sigma,  \ol{Y} \triangleleft \ol{P})}\commentarymark &= \unify{}(\consSet) & \text{// constraint solving}\\
...
The overall algorithm is nondeterministic. There may be multiple solutions for a constraint set and a local solution for class $\mv L$ may not
be compatible with the constraints generated for a subsequent class. In this case, we have to backtrack to $\mv L$ and proceed to the next
