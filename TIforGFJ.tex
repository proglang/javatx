...
\item Substitute \commentarymargintext{principal type}{gibt es nicht mehr} solution
\end{enumerate}
...
\begin{figure}[tp]
  \begin{align*}
    ...
    \typeAssumptionsSymbol &::= (\commentaryintext{\mv{\Pi}}{Was ist jetzt $\Pi$?}\ \overline{\methodAssumption} ; \overline{\fieldAssumption}; \overline{\localVarAssumption}) 
  \end{align*}
  \caption{Syntax of onstraints}
...
an or-constraint $\orCons$, which is a set of set of simple
...
consists of the method type environment \commentarymargintext{$\mv\Pi$}{Aha hier kommt
    $\Pi$. Sollte vorher erklärt werden.} of the previously
...
\begin{gather*}
  \begin{array}{@{}l@{}l}
    \typeExpr{} &(\commentarymark{\typeAssumptions}, \texttt{e}.\texttt{f}) = \\
                & \begin{array}{ll}
                    \textbf{let} 
  ...
  \end{array}
\end{gather*}
\commentary{$(\overline{\methodAssumption} ; \overline{\fieldAssumption}; \overline{\localVarAssumption})$? Das gilt für alle weiteren Funktionen auch!!!}
...
(\texttt{new Classname(...)} instead of \texttt{new Classname<}\replaced{Class}{$\overline{R}$}\texttt{>(...)}).
...
$\ol{X}$ can occur in the bound $\ol{N}$ as well as in the types of the constructor parameters $\ol{T}$.
