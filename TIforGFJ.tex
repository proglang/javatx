...
\item Setting in one of the type solutions with \fjtypeInsert{}
\end{enumerate}

The complete $\fjtypeinference{}$ algorithm for a single class can be given as follows:
$\fjtypeinference : \mv{\Pi} \times \texttt{L} \to \mv{\Pi}$\\
$\begin{array}{l}
\fjtypeinference(\mv{\Pi}, \texttt{L}) = \\
...
In the third step of the algorithm we set in one solution at a time.
Here we describe the third step as a nondeterministic function, which picks the right solution.
It is possible that subsequent classes resolve some of the solutions as incorrect.
In praxis we can solve this problem by backtracking and trying out every possible solution one by one if needed.
...
\begin{theorem}\textbf{Completeness:}
  If there exists a $\mv{\Pi}'$ and $\texttt{L}$ with $\mv{\Pi} \vdash \texttt{L} : \mv{\Pi}'$, then $\textbf{FJTypeInference}(\mv{\Pi},\texttt{L}) = \mv{\Pi}''$ with $\mv{\Pi} \vdash \texttt{L} : \mv{\Pi}''$.
\end{theorem}
\textbf{Proof:}
