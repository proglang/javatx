...
%Each cast could either be a upcast or a downcast.
%We generate a \textbf{OrConstraint} to cover both.

%$\begin{array}{@{}l@{}l@{}l}
%\textbf{TY}& \textbf{PE} & \textbf{Expr} (Ass, (N)e ) = \\
%& \textbf{let} % \\
%& (t, ConsE) = \textbf{TYPEExpr}(Ass, e),\\
%& & Cons = \textbf{OrConstraint}(\set{t \lessdot N}, \set{N \lessdot t}) \\
%& \mathbf{in}% \\
%% &
%& (N, (ConsE \cup Cons))\\
%\end{array}
...
\begin{tabularx}{\linewidth}{lX|Xl}
  ...
  $\mathtt{\textit{mtype}(\ldots) = \exptype{}{\ol{Y} \triangleleft}\ \ol{U} \to U}$ &&& $rty \lessdot \exptype{C}{\textbf{fresh}(\ol{X})}$ \\
