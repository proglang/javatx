...
\title{Type-infered Featherweight Generic Java} %TODO Please add
...
\Copyright{A. Stadelmeier and M. Plümicke and P. Thiemann} %TODO mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/
...
The solver returns a set of possible solutions for each method.
The last step chooses the principal type out of those possibilities.
\todo[inline]{PT: I thought the set of solutions constitutes the
  principal type. What's there to choose?}

\subsection{Process multiple classes}
\label{sec:proc-mult-class}

Type inference processes one class at a time.
...
and \mv{m2} are mutually recursive across class boundaries.
In contrast, Figure~\ref{fig:correctinput} contains a program
acceptable to both \TFGJ and FGJ because the mutual recursion of
methods \mv{m1} and \mv{m2} is taking place inside class \mv{C2}.
...
& \multicolumn{2}{@{}l@{}}{ \set{ T \lessdot U \  | \  (m:\ \ol{T} \to T, m_o:\ \exptype{}{\ol{Y} \triangleleft \ol{P}} \ol{U} \to U) \in \overline{O}}} \\
& & \text{with:}\ \overline{O} = \set{(m, m_o) \  | \  m \ \text{overrides method}\ m_o} \\
\end{array}$
