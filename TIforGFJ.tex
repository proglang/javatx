...
\begin{figure}[tp]
  \begin{align*}
    ...
    \methodAssumption &::= N.\texttt{m} : \exptype{}{\ol{X}
                        \triangleleft \ol{N}}\ \ol{T} \to T  &&
                                                                \text{method
                                                                type assumption}\\
    \fieldAssumption &::= N.\texttt{f} : T && \text{field type assumption}\\
    \localVarAssumption &::= \texttt{x} : T && \text{parameter assumption}\\
    \typeAssumptionsSymbol &::= (\overline{\methodAssumption} ; \overline{\fieldAssumption}; \overline{\localVarAssumption}) 
  \end{align*}
...
constraints. An or-constraint represents different alternatives and
...
\todo[inline]{PJT 20220214: field type assumptions should have
  the form $\exptype C{\ol X} : \mv T$ (where $\mv T$ may contain the
  $\ol X$) as field types are not inferred. In particular, they cannot
  contain type variables $\tv{a}$. Is that correct?}
\todo[inline]{PJT 20220214: do not change the
  meaning of previous symbols. In this case, the new definitions for
  $\mv\Gamma$ and $\mv\Pi$ conflict with previous ones. It would be
  better (and easy) to adapt the definition so that both can live
...
\noindent
$\fjtype : \typeAssumptionsSymbol \times \texttt{L} \rightarrow \ol{C}\\
 \begin{array}{@{}l@{}l}
  ...
  & \begin{array}{l@{}l@{}l}
    \textbf{let} &\ \ol{\methodAssumption}_o = \set{\exptype{C}{\ol{X}}.\mathtt{m} : \exptype{}{\ol{Y} \triangleleft \ol{P}}\ \ol{T} \to \tv{a} \ |\ \texttt{m} \in \ol{M},
    \texttt{m}\ \text{overrides}\ \texttt{N}.\texttt{m} : \exptype{}{\ol{Y} \triangleleft \ol{P}} \ol{\texttt{T}} \to \texttt{T}}\\
    &\ \constraints_o = \set{\tv{a} \lessdot \texttt{T} \ |\ \texttt{m} \in \ol{M}, 
    \texttt{m}\ \text{overrides}\ \texttt{N}.\texttt{m} : \exptype{}{\ol{Y} \triangleleft \ol{P}} \ol{\texttt{T}} \to \texttt{T}}\\
    &\quad \quad \textbf{with}\ \tv{a}\ \text{being the same fresh type variable in both cases} \\
    %\textbf{let} &\ m_{cons}, m_{override} = \set{\overridesFunc(\typeAssumptions, \exptype{C}{\ol{X}}.\mathtt{m}) \ |\ m \in \ol{M}} \\
    &\ \methodAssumption' = \begin{array}[t]{l}
     \set{ (\exptype{C}{\ol{X}}.\mathtt{m} : \ol{\tv{a}} \to \tv{a}) \ |\ m \in \ol{M},\\
     \quad \quad \texttt{m}\ \text{does not override another method} \\
     \quad \quad \tv{a}, \ol{\tv{a}}\ \text{are fresh type variables} } \\
     % \cup \\
     %\typeAssumptions \cup m_{override}
    \end{array}\\
  \textbf{in} \\
   &\ \displaystyle{ \left( \bigcup_{\texttt{m} \in \ol{M}} \typeMethod( (\ol{\methodAssumption} \cup \ol{\methodAssumption}' \cup \ol{\methodAssumption}_o , \ol{\fieldAssumption}, \{ \mathtt{this} : \exptype{C}{\ol{X}} \}), \mathtt{m}) \right)
   \cup \constraints_o } \\ 
  \end{array}
\end{array}$
...
\begin{array}{@{}l@{}l@{}l}
\typeMethod & (\typeAssumptions,\ & \type{T} \ \mathtt{m}(\overline{\texttt{T}} \ \overline{\texttt{x}})\{ \mathtt{ return }\ \texttt{e}; \}) =\\
& \textbf{let}
& (\texttt{e}:\type{R}, \constraints) = \typeExpr((\overline{\methodAssumption} ; \overline{\fieldAssumption}; \overline{\localVarAssumption} \cup \{ \overline{\texttt{T}} : \overline{\texttt{x}} \}), \texttt{e})\\
& \mathbf{in}
& \constraints \cup \set{\type{R} \lessdot \type{T}}\\
\end{array}
$
...
\typeExpr ((\overline{\methodAssumption} ; \overline{\fieldAssumption}; \overline{\localVarAssumption}\cup \set{\mathtt{x} : \type{T})}, \mathtt{x}) = (\type{T} , \emptyset)
$
...
& \begin{array}{ll}
\textbf{let} 
& (\type{R}, \consSet_e) = \typeExpr(\typeAssumptions, \texttt{e}),\\
& \consSet_f = \set{\type{R} \lessdot \exptype{C}{\ol{X}}}\\
...
\textbf{let}& % \\
% &
(\type{R}, \constraints_R) = \typeExpr(\typeAssumptions, e),\\
& \forall e_i \in \overline{e} : (pt_i, \constraints_i) = \typeExpr(e_i)  ,\\
& \begin{array}{@{}l@{}l}
  \constraints_{m} = \{\ & \set{ \type{R} \lessdot \exptype{C}{\ol{X}}, \tv{a} \doteq \type{T}, \bigcup_{T_i \in \overline{T}} (pt_i \lessdot T_i)
        , \ol{Y} \lessdot \ol{P} }\\
                    & |\, \text{for every method}\ \exptype{}{\ol{Y} \triangleleft \ol{P}} T\ \texttt{m}(\ol{T}\  \ol{p}) \in \ol{M} \} \\%, \textbf{fresh}(\ol{X}) \lessdot \textbf{fresh}(\ol{N})\},\\
                    & ,\, \text{in every class}\ \exptype{C}{\ol{X}} \{ \ldots \ol{M} \ldots \}\ \text{in the input} \} 
\end{array}\\
& \constraints = \orCons(\constraints_{m})\\
\multicolumn{2}{l}{\mathbf{in} (a, [\textbf{fresh}(\ol{X})/\ol{X}][\textbf{fresh}(\ol{Y})/\ol{Y}](\constraints_R \cup \bigcup_i \constraints_i \cup \constraints))}\\
...
& \constraints = \{ \bigcup_{T_i \in \overline{T}} (\type{P}_i \lessdot \type{T}_i) \ | \ \exptype{C}{\ol{X}}.\texttt{f}_i : \type{T}_i \}\\
