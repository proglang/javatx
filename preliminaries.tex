...
omitted.%, except the ones for casts and \texttt{new} calls. 
\todo[inline]{``Typeless'' does not sound right. How about ``Untyped''?
or call the language FGJ-GT (FGJ with global type inference)?}
...
\todo[inline]{PT: why can't we treat ``this'' in the same way as FGJ,
  which just puts the correct type assumption in the environment?}
